...
comment on function public.get_dashboard_metrics is
  'Backs the Management Dashboard''s summary cards. security definer so '
  'Staff (whose role cannot SELECT from bookings directly) still gets '
  'these counts, matching the mockup — but only ever aggregate numbers, '
  'never a guest name or specific booking detail.';
